...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	//static TCHAR tempstr[20];
	static bool isClick = false;
	static int mX1, mY1, mX2, mY2, count = 0, line = 0;
	static int Figure = 0;
	static TCHAR str[20][200];
	//static RECT temprect = { 0,0,100,100 };
	//static RECT temprect2 = { 30,30,100,100 };
	//static RECT temprect3 = { 60,60,200,200 };
	switch (iMsg)
	{
	case WM_CREATE:
		break;
	case WM_CHAR:
		hdc = GetDC(hwnd);
		/*
		_stprintf_s(tempstr, _T("%ld"), size.cx);
		DrawText(hdc, tempstr,_tcslen(tempstr),&temprect,DT_CENTER);
		_stprintf_s(tempstr, _T("%d"), FONT.iPointSize);
		DrawText(hdc, tempstr, _tcslen(tempstr), &temprect2, DT_CENTER);
		*/
		if (wParam == VK_RETURN)
		{
			//str[line][count] = '\n';
		...
		size.cx *= FONT.iPointSize / 100.0f;
		if (size.cx >= rect.right - rect.left - FONT.iPointSize/10.0f)
		{
			//str[line][count] = '\n';
			line++;
			count = 0;
	...
	case  WM_COMMAND:
		...
		case ID_FONT:
			...
			if (ChooseFont(&FONT) != 0)
			{
				...
				hFont=CreateFont(LogFont.lfHeight, LogFont.lfWidth, LogFont.lfEscapement, 
